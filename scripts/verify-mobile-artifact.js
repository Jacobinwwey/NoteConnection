...
function verifyMobileArtifact(options = {}) {
  ...
  return {
    artifactPath,
    artifactKind: artifactPath.toLowerCase().endsWith('.aab') ? 'aab' : 'apk',
    profile: profileId,
...
function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('Usage: node scripts/verify-mobile-artifact.js --artifact <file.apk|file.aab> [--profile mobile-low|mobile-standard] [--rss-evidence <file>] [--require-rss] [--require-arm64]');
    return;
  }
...
module.exports = { PROFILES, FORBIDDEN_ENTRY_PATTERNS, listZipEntries, verifyMobileArtifact };
