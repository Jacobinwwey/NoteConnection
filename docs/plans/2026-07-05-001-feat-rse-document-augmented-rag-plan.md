...
- Add tests for:
  ...
  - controlled location conflicts through `conflicting_location_evidence_probe_en` and temporal/current-vs-historical false-positive guards through `temporal_scoped_location_probe_en` and `temporal_scoped_ownership_identity_probe_en`;
  ...
  - controlled dependency/package/provider conflict and environment-qualified dependency false-positive coverage through `conflicting_dependency_evidence_probe_en` and `environment_scoped_dependency_probe_en`;
