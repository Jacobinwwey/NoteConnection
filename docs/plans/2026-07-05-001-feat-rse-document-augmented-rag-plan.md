...
- Add tests for:
  ...
  - conflicting adjacent and same-section non-adjacent measurement/date evidence;
  ...
  - graph-neighbor source-window unavailability through `graphintent_missing_neighbor_source_window_en`;
...
- Runtime: `graphintent_compare_neighbor_selection_en` and `graphintent_missing_neighbor_source_window_en` require graph diagnostics to report at least two intent-aligned successor candidates, at least one intent-misaligned successor candidate, and no misaligned-successor fallback.
...
- `graphintent_compare_neighbor_selection_en` 与 `graphintent_missing_neighbor_source_window_en` 会要求 graph diagnostics 至少报告 2 个 intent-aligned successor candidate、至少 1 个 intent-misaligned successor candidate，并确认没有使用 misaligned-successor fallback。
