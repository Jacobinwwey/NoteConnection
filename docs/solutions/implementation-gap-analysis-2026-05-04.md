...
| **M10.6** ANN 生产连接器 | 完整向量检索闭环 | `queryBackend.ts` AnnRunbookHealthGate + prefilter effectiveness + representation validation + `vectorAccelerationAdapter.ts` prefilter metrics | 🟢 **Done v2.5** | 生产闭环完成 |
