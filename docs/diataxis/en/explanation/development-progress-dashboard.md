...
- browser smoke now exercises real `conversation/path/query-compare/quality/session/runbook` backend slices (including trend + history diagnostics plus runbook checks/action-queue), real graph runtime, and real path runtime, and emits screenshot/console/network-summary evidence paths (`scripts/verify-agent-workspace-browser.js`, `src/agent_workspace.browser.contract.test.ts`),
...
- Key observability integration:
  ...
  - dedicated vector acceleration governance drilldown panel in the runbook section (structured status/threshold/flag/action view for `query_vector_acceleration_circuit_state`, prefilter selection/candidate telemetry + threshold budget/action view for `query_vector_acceleration_prefilter_effectiveness`, plus traceability coverage/action view for `query_vector_acceleration_traceability`)
...
2. Move M10 from rollout bootstrap to foundation closure by replacing the current file-backed graphdb adapter path with a real graph backend adapter, while preserving fail-open/fail-closed rollout semantics.
3. Promote ANN from scaffold mode (`ann_prefilter` + `external_*`) to a production connector baseline with health checks, timeout/retry/circuit controls, and explicit fallback observability.
4. Add CI-integrated durability checks for persisted turn-cache trend index/export consistency across restart flows.
5. Continue strict evidence artifact governance improvements (retention/indexability/export) for operator audits.
6. Keep i18n expansion for long-lived structured conversation artifacts as a lower-priority stream unless it directly unblocks contract/foundation risk.
