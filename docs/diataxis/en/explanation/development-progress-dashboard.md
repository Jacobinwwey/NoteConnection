...
- `src/learning/conversationComposer.ts` now applies a compare RAG answer profile that keeps two direct-support clauses when the pack contains evidence for both compared sides, then adds bounded graph-neighbor contrast context without emitting extra chat messages.
...
- Expand runtime probes for provider timeout/fallback, malformed judge JSON, intent-specific graph-neighbor selection, and large-corpus hard negatives.
- Expand the answer-profile system beyond the current RAG-aware definition and compare paths while preserving hard public-answer budgets.
