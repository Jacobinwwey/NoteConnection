...
- Extend replay tooling beyond stable context ids, failure-stage classifications, and answer claim-citation maps; add larger runtime probes for repeated snippets, broader conflict patterns beyond adjacent numeric facts, multi-document missing graph-neighbor evidence, stricter total-character budget drop cases, and provider fallback.
