...
- The runtime probes `conflicting_format_evidence_probe_en` and `environment_scoped_format_probe_en`, backed by `Knowledge_Base/ragformatconflict` and `Knowledge_Base/ragformatqualifier`, now extend controlled semantic-conflict coverage to explicit format/schema/encoding/serialization/content-type/mime-type facts. Same-section JSON versus YAML payload format statements produce a `conflict` fragment and `context_assembly` degradation, while staging JSON and production XML payload formats stay environment-qualified evidence without a false conflict.
