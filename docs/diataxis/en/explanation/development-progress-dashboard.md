...
The report assembler also normalizes flattened source fragments such as `### Section body...`: it recovers the heading/body boundary from `headingPath`, while standalone headings remain pending until a real body arrives. This prevents source-window truncation from producing either lost technical content or a dangling final heading.
...
The mobile profile is additive and bounded. JSON, live SSE, and SSE replay all accept `responseProfile: mobile_compact` and return only `mobileProjection`, a bounded summary, and empty internal collections; `trace`, RAG fragments, knowledge-run artifacts, assistant metadata blocks, and raw source bodies are excluded. The turn-cache fingerprint includes the response profile, so a desktop replay cannot be reused as a mobile response. Native Capacitor/Android conversation prefers the local exact index, extracts CJK/English subject terms from compound prompts, reads at most one bounded source document, and derives the route through the shared `mobile_exact_analyzer` explicit-edge contract. A no-match or unavailable local index falls back to the same bounded remote projection. `note://workspace/v1/...` source URIs are mapped to `Knowledge_Base/...`; unknown or absolute desktop URIs fail closed.

Fresh verification against the current build is green: TypeScript no-emit; full Jest `155` suites / `1,356` passed / `26` skipped; production build; Vite build; Agent Workspace contracts `157` passed / `13` skipped; mobile contracts `51` passed; projection replay across `4` hosts; native recovery `8` scenarios; mobile slim `122` files / `4,292,745` uncompressed bytes / `1,554,525` estimated compressed bytes under the 25 MiB budget; and the real backend/browser verifier. The isolated browser run used a real backend, real graph runtime, and Godot future-path runtime. The fresh `waterglass` runtime run retained the full internal evidence for audit while releasing a bounded answer with balanced, single-instance thermal and Snell-law equations, no Mermaid/comparison/application scaffolding, and the typed learning-route deliverable. Evidence files under `output/verification/` and `output/playwright/` are verification artifacts, not product assets.

The remaining release blockers are unchanged and explicit: this host has no online Android target (`adb devices -l` is empty), so signed arm64 APK/AAB installation, SAF workload, process-death/reopen continuity, and device RSS are not claimed. The already-open legacy NoteConnection process still runs its old build and was not modified; operator restart/rebuild is required before it can load this code. Remote GitHub CI is the final gate after the commit is pushed.

## 2026-08-25 Fresh Browser Acceptance and Evidence Boundary

The browser acceptance harness was rerun from a freshly started isolated Chrome after the CDP target was re-probed against the mandatory `http://127.0.0.1:12345` browser proxy. The harness now waits for and closes the asynchronous first-run language selector before opening the workspace drawer, then asserts that the drawer is actually open before changing scope and sending the request. This closes a verifier timing race that otherwise left the UI idle and falsely reported a missing answer card.

The fresh run exercised a temporary server, temporary runtime data, and a temporary `Knowledge_Base` through the real ingest -> HTTP/SSE conversation -> browser DOM path. It produced one `Grounded Answer`, KaTeX count `4`, complete thermal and Snell-law TeX annotations, zero raw math delimiters, no mounted `Answer Context`/`Explanation`/`Evidence Summary`, zero visible Knowledge Run cards, no visible Knowledge Actions, and a collapsed `Next Actions` control (`aria-expanded=false`, no panel). The released answer contains one complete public definition; the audit plan intentionally retains the malformed `... across t` candidate to prove that the audit/public boundary is real. CDP evidence is recorded in `output/browser-research/agent-answer-formula-render-latest.json` and the paired PNG; those are external verification artifacts, not product assets.
...
That probe found two real boundaries that focused tests had not covered. First, a contextual formula claim and a formula-only block could both pass the definition projection, producing the same equation twice. `answerReleaseReview.ts` now deduplicates only formula-only public claims whose normalized equation is already carried by an earlier contextual claim; internal `GraphAnswerPlan`, evidence refs, and audit trace remain unchanged. Second, `server.ts` had a legacy private conversation normalizer that dropped `answerLanguage`, so direct platform calls behaved correctly while the actual HTTP/SSE route silently reverted to automatic language detection. The route now reuses `learning/requestNormalization.ts` as its single request boundary.

The same audit exposed an idempotency flaw: the turn-cache fingerprint had included only user, message, top-K, and time. It now includes normalized answer language, session, active target, persistence policy, memory namespace, and a sorted resolved scope. A caller cannot replay a cached turn under another answer-language or scope contract. `src/server.migration.test.ts` covers the Chinese structured block through the real route, an identical replay, and rejection of a language/target-mismatched replay.

Language behavior is deliberately split by evidence authority. The selection controls backend-generated structured titles, `Next Actions`, and other system-authored copy; it is persisted client-side and sent as `answerLanguage`. Source-backed prose stays in the source language when no configured translation provider exists. This is intentional: silently translating technical evidence would create an ungrounded second-generation answer. An opt-in provider-backed translation layer remains a separate product decision with source/translation provenance requirements.

## 2026-08-24 Public Projection and Final-Coverage Follow-up

The incident analysis found a release-boundary gap in the first closure: coverage was checked against a projected plan before revision, while the public answer could still be changed afterward. `AnswerReleaseReview` now carries the exact public projection and the coverage result evaluated against the exact text released to clients. `conversationComposer.ts`, the response trace, and the knowledge-run artifact reuse that result instead of recomputing coverage against the unprojected audit plan.

This is deliberately not a blind fallback. A public plan is activated only when it is actually part of the release contract: a definition answer needs projection because its candidate is incomplete, malformed, or contains documentary/comparison artifacts, or a RAG-backed answer uses its planned evidence. Temporal and graph-consistency corrections remain authoritative and suppress the public plan rather than being overwritten by a coverage repair. If an active public plan loses required coverage after revision, release reconstructs the safe projected plan; if that still cannot cover its required claims, the answer abstains rather than emitting a partial answer with a passing-looking trace.

Definition projection now has no fixed claim-count or character ceiling. It keeps every eligible, complete, balanced-math claim from the already bounded evidence plan, removes comparison/document artifacts, and removes formula-only duplicates only when a contextual claim already carries the same normalized equation. Conflict degradation no longer admits raw supplemental claims, so it cannot reintroduce comparison prose or duplicate equations. Explicit `answerLanguage` now reaches the no-match and abstention paths as well as ordinary grounded replies; JSON and SSE contract tests cover an English query with a Chinese response contract.

## 2026-08-23 Agent Workspace Answer-Quality Incident Closure

The first live-process probe was intentionally treated as evidence, not as a passing check. The running NoteConnection instance (server PID 26892, Godot PID 27264, parent npm PID 39156, HTTP/bridge ports 12847/12848) reproduced the reported failure: the legacy build emitted a 45-claim internal graph plan as one public definition answer, leaked comparison-table neighbors, preserved structural headings such as `4.`, and returned Markdown math as raw text to the structured-answer renderer. WebView2 CDP port 1665 was not enabled for that existing process, so no false CDP claim was made.

The forward-compatible fix keeps the internal `GraphAnswerPlan` and trace complete, but adds a definition-intent public projection at `answerReleaseReview.ts`: comparison/analogy and documentary/navigation artifacts are excluded; incomplete numbered or clause tails are rejected; public claims have no arbitrary count or character ceiling; and coverage is evaluated against the exact public projection only when that projection is active. Evidence highlights no longer hard-cut at 120 characters. They are emitted only at a complete sentence boundary with balanced `$...$`/`$$...$$` delimiters, otherwise omitted.

The frontend `structured_answer` path renders only `directAnswer` through the existing `marked -> sanitize -> KaTeX -> Mermaid` pipeline. `Answer Context`, `Explanation`, and `Evidence Summary` remain response metadata and are not mounted into the chat DOM. `Next Actions` stays behind a small circular toggle and is rendered only after user intent. This preserves the additive response contract and keeps mobile payloads free of model weights or extra runtime dependencies.

Fresh evidence: focused answer regression, the full `src/learning` suite, frontend workspace suite (107 tests), TypeScript no-emit, production build, sidecar validation, isolated runtime shell verification, and a rebuilt `dist` server on an independent port with the real Water Glass corpus. The rebuilt server returned a bounded, complete formula-bearing answer without `Plastic Cup`, `Metal Cup`, Mermaid fences, `4.`, or an ellipsis, while retaining the full internal plan for audit. The currently open legacy process remains untouched and must be restarted/rebuilt by the operator before it can load this fix.
...
- Current host evidence: migration matrix 57 suites (307 passing tests), focused projection/Bridge suites, Rust graph-runtime tests, PathBridge strict, Diataxis, and TypeScript build passed; fresh staging measures 120 files, 4,253,837 bytes uncompressed and 1,546,201 estimated compressed bytes.
...
Migration matrix 57 suites / 307 tests passed, alongside focused projection/Bridge tests, Rust graph-runtime tests, PathBridge strict, Diataxis, and slim staging. The fresh staged profile is 120 files, 4,253,837 uncompressed bytes, and 1,546,201 estimated compressed bytes. Registry response/status shadow parity, signed arm64 APK extraction, device RSS, Android folder import, cross-host replay, and canonical public-ID cutover remain open; `not-measured` is not a pass.
...
## 2026-08-17 Phase 10 Projection and Host Execution
...
- Fresh mobile-slim evidence is 120 files / 4,253,837 uncompressed bytes / 1,546,201 estimated compressed bytes. Signed arm64 APK/AAB, device RSS, Android folder import, and cross-host replay remain open gates.

## 2026-08-18 Phase 12 Projection Store and Android SAF
...
- Android SAF import now persists an atomic v1 journal and recovers interrupted staging/backup activation on activity startup; corrupt or path-escaping journals fail closed.
