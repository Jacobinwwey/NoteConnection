...
- strict browser verification in `scripts/verify-agent-workspace-browser.js` now owns the user-reported `water glass.md` UI regression surface, including hosted Godot Future Path `diffusion/core` projection for `water glass`, TreeRenderer markers/hulls/viewport auto-fit, refusal to dock `#path-container`, refusal to call bridge/Tauri `showGodot=true`, refusal to dock the real `#graph-container`, high-density Focus context dots, and a hosted Focus anchor assertion for `water glass`.
...
| `Related Focus` should correspond to the Tauri Focus-mode state | The pane hosts Focus-mode behavior locally through `getFocusModeProjection()`: active Focus nodes, toolbar/control metadata, high-density background context nodes, pane-local double-click switching, pane-local Markdown reader, no mutation of the main graph runtime; diagnostics are Developer Mode only. | Implemented |
| `Learning Path` should correspond to the Godot/Path-mode design | The pane hosts the Godot Future Path data contract by using the existing frontend `Graph` / `PathEngine` `diffusion/core` + `treeLayout` path for the resolved DAG node, then renders it through `godot_future_path_renderer.js`; it deliberately does not mount browser `path-container` or show the native Godot window by default. | Implemented |
