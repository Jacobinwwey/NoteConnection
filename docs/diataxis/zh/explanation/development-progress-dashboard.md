...
- runtime probe `conflicting_protocol_evidence_probe_en` 与 `environment_scoped_protocol_probe_en` 现在由 `Knowledge_Base/ragprotocolconflict` 和 `Knowledge_Base/ragprotocolqualifier` 支撑，把受控语义冲突覆盖扩展到显式 protocol/transport-protocol/wire-protocol facts。同一 section 中 HTTP/1.1 / WebSocket transport protocol 会产出 `conflict` fragment 并在 `context_assembly` 下退化；staging HTTP/2 与 production gRPC transport protocol 会保留为 environment-qualified evidence，不会误产出 conflict。该覆盖刻意窄于 endpoint compatibility 或 route-version 推断。
