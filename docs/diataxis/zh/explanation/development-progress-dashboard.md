...
| 弱证据应显式降级 | `RagSufficiencyReview` 现在记录 `sufficient`、`borderline` 或 `insufficient`，并带有 `partial_coverage`、`conflict`、`insufficient_evidence` 等降级状态；`evidenceContextAssembler.ts` 会把相邻或同一 scoped section 内非相邻、同主语但数值冲突的证据发出为 `conflict` fragment，也会把图邻居 source-window 不可用的 source decision 标注到 `graph_neighbor_support`；`ragSufficiencyJudge.ts` 会在图上下文存在但邻居缺少可恢复源窗口时输出 `graph_neighbor_evidence_missing`，避免仅凭图邻居标题或 direct span 通过充分性；前端 evidence pane 会显示 compact 状态。 | 确定性 review 已实现 |
