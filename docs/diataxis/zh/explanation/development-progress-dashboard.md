...
本切片现在记录更充分 Knowledge Workspace 回答的实现进展。具体方案仍以 [RSE document-augmented graph RAG answer pipeline](../../../plans/2026-07-05-001-feat-rse-document-augmented-rag-plan.md) 为准，但当前分支已经不只是规划：确定性的 RSE / document augmentation 链路、query-intent 图邻居排序、有界 context pack、接入 provider 的充分性 trace、有界一次性 recovery、RAG-aware 单消息 release review、确定性的 compare answer-profile 预算、runtime verifier 字段、前端 compact RAG 状态，以及导出 RAG trace 保留已经落地。how-to / generic answer profile、图置信阈值校准、更丰富 replay id 和更大的 runtime probe 语料仍是后续工作。
...
- `answerReleaseReview.ts` 负责 public-surface contraction、RAG answer completeness 与最终公开回答修订。
...
- `src/learning/conversationComposer.ts` 现在应用 compare RAG answer profile：当 context pack 同时包含被对比双方的 direct support 时，公开回答会保留两边证据，再加入有界 graph-neighbor contrast context，同时不额外产生聊天消息。

后续推进：

- 基于代表性 hard negative 校准图关系权重与低置信排除阈值，不把当前 scoring 常量当作最终形态。
- 扩展 provider timeout/fallback、malformed judge JSON、intent-specific 图邻居选择与大语料 hard negative 的 runtime probes。
- 将当前 RAG-aware definition / compare 路径继续扩展为完整 answer-profile 系统，同时保留公开回答硬预算。
