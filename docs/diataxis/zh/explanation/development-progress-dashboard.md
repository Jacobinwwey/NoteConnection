...
- runtime probe `graphintent_compare_neighbor_selection_en` 现在验证 `compare brittle glass vessel with polymer cup material behavior` 会从 `Knowledge_Base/graphintent` 选择 analogy graph successors，从 `successorWindow` 排除高重叠 procedural successor，保留 `graph_neighbor_support`，并且不会把 procedural path 泄漏进公开回答。
