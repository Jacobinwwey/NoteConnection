...
- runtime probe `conflicting_dependency_evidence_probe_en` 与 `environment_scoped_dependency_probe_en` 现在由 `Knowledge_Base/ragdependencyconflict` 和 `Knowledge_Base/ragdependencyqualifier` 支撑，把受控语义冲突覆盖扩展到显式 dependency/package/provider/driver/runtime/library/module/plugin/adapter facts。同一 section 中 SQLite 与 PostgreSQL storage dependency 会产出 `conflict` fragment 并在 `context_assembly` 下降级；staging 与 production dependency records 会保留为 environment-qualified evidence，不会误产出 conflict。
