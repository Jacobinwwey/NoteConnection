...
- 项目现有 DAG 已经被使用了两次：
  ...
  - release-review 阶段由 `claim_graph_causal_consistency` 与 `claim_graph_order_consistency` 消费，
...
| 现有 DAG 必须参与回答正确性，而不只是 retrieval 排序 | `graphContextAssembler.ts` 在 synthesis 前装配图上下文，`claim_graph_causal_consistency` 与 `claim_graph_order_consistency` 会在 release 时消费 DAG 证据。 | 已实现基线 |
