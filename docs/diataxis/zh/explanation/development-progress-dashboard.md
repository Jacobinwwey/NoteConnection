...
| 弱证据应显式降级 | `RagSufficiencyReview` 现在记录 `sufficient`、`borderline` 或 `insufficient`，并带有 `partial_coverage`、`conflict`、`insufficient_evidence` 等降级状态；`evidenceContextAssembler.ts` 会把相邻或同一 scoped section 内非相邻、同主语但 measurement、unitless quantity/limit、date/year、显式 state/status 或受控 location 值冲突的证据发出为 `conflict` fragment，也会把图邻居 source-window 不可用的 source decision 标注到 `graph_neighbor_support`；`ragSufficiencyJudge.ts` 会在图上下文存在但邻居缺少可恢复源窗口时输出 `graph_neighbor_evidence_missing`，避免仅凭图邻居标题或 direct span 通过充分性；前端 evidence pane 会显示 compact 状态。 | 确定性 review 已实现；更广语义类仍未关闭 |
...
- runtime probe `conflicting_format_evidence_probe_en` 与 `environment_scoped_format_probe_en` 现在由 `Knowledge_Base/ragformatconflict` 和 `Knowledge_Base/ragformatqualifier` 支撑，把受控语义冲突覆盖扩展到显式 format/schema/encoding/serialization/content-type/mime-type facts。同一 section 中 JSON / YAML payload format 会产出 `conflict` fragment 并在 `context_assembly` 下退化；staging JSON 与 production XML payload formats 会保留为 environment-qualified evidence，不会误产出 conflict。
