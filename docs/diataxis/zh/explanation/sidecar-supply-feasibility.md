...
| 用 GitHub Releases 作为第一镜像 | 通常是最低额外成本 | 低，只要不把下载责任推给运行时用户 | 低 | 很强 | 当前 release workflow 已会维护专用 Godot 镜像 tag，也已经对归档做固定 SHA256 校验，并可在 mirror-only smoke 中关闭上游回退；只是默认仍保留它 | 当前最合适 |
...
- 桌面 bundle job 现在默认会以“镜像优先、上游回退”方式下载 Godot
- 同一条 workflow 现在也可以通过 `allow_godot_upstream_fallback=false` 进入 mirror-only smoke 校验

仍然缺的部分：

- 先把 mirror-only smoke 变成常规验证，再移除默认上游回退
