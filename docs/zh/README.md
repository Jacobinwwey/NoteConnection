...
- 公开回答审核层现在已经是实际运行时 owner，而不是未来方案占位：`src/learning/answerReleaseReview.ts` 会在 scoped retrieval 与 graph-context assembly 之后执行，负责从 public surface 剥离内部诊断、校验证据充分性，以及 lexical / query-intent / structured / containment / subject / state / polarity / graph-causal / graph-order 一致性，并把 gate 结果持久化到 conversation trace / export 流程中。
- 右侧证据预览现在也有了真实的 source-to-render 基线，而不再只是 snippet 猜测：`src/frontend/markdown_runtime.js` 会给渲染后的 markdown block 标注 source-line 元数据，`src/frontend/workspace_panes.js` 会优先使用 `source_line_provenance`，其次才是可信 `line_window`，最后才回退到 `snippet_fallback`；在选中正确节点后，系统现在还会把命中的证据片段投影成内联高亮，而不再只是整段着色。
- 截图 `E:\微信传输\WeChat Files\wxid_zywfbxxiwwir22\FileStorage\Temp\1781782257390.jpg` 对应的失败路径现在已经落成正式运行时验收用例：`src/learning/KnowledgeWorkspaceConversationRegression.ts` 中的 `waterglass_explicit_scope_compact_zh` 要求至少返回 1 条 citation，明确禁止在 public answer 泄漏 `No scoped knowledge points matched` / `retrieval_candidates_below_threshold`，并且不允许继续把 `本技术文档旨在` 这类文档自述放到主回答里。
- 当前缺口已经不再是“没有 reviewer owner”，也不再是“完全没有 provenance”。真正剩余的是超出当前 lexical + query-intent + structured + containment + subject + state + polarity + graph-causal + graph-order 栈的更广 claim-vs-citation / claim-vs-evidence 矛盾检测，以及超出当前 block-level markdown source mapping 与 snippet-projected 内联高亮基线的 source-authenticated 字符级 provenance。
- `ref/dspy`、`ref/guidance`、`ref/semantic-kernel`、`ref/langchain` 与 `ref/litellm` 继续保留为设计参考，而不是运行时所有权归属。可借鉴的是 typed LM subprogram、受约束输出契约与评测回路；明确拒绝的是把 Python-heavy 或 orchestration-heavy 框架直接塞进已经自带 retrieval、graph assembly、export 与 UI compatibility 责任的 TypeScript DAG runtime。Semantic Kernel 当前仓库朝 Microsoft Agent Framework 收敛、LiteLLM 主要是 provider gateway，这两点也进一步说明它们不能替代本地 release verification owner。
