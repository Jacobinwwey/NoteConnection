...
- 公开回答审核层现在已经是实际运行时 owner，而不是未来方案占位：`src/learning/answerReleaseReview.ts` 会在 scoped retrieval 与 graph-context assembly 之后执行，负责从 public surface 剥离内部诊断、校验证据充分性，以及 lexical / structured / state / polarity / graph-order 一致性，并把 gate 结果持久化到 conversation trace / export 流程中。
- 右侧证据预览现在也有了真实的 source-to-render 基线，而不再只是 snippet 猜测：`src/frontend/markdown_runtime.js` 会给渲染后的 markdown block 标注 source-line 元数据，`src/frontend/workspace_panes.js` 会优先使用 `source_line_provenance`，其次才是可信 `line_window`，最后才回退到 `snippet_fallback`，并把高亮 / provenance 诊断作为 additive 信息暴露给运维面。
...
- 当前缺口已经不再是“没有 reviewer owner”，也不再是“完全没有 provenance”。真正剩余的是超出当前 lexical + structured + state + polarity + graph-order 栈的更广 claim-vs-citation / claim-vs-evidence 矛盾检测，以及超出当前 block-level markdown source mapping 的更深 exact-span / nested provenance。
