...
- [x] 已把 reviewer 从 lexical + structured-fact 检查继续扩展到确定性的 `claim_dependency_consistency`，覆盖 `依赖基线测量和传感器校准` vs `依赖最终报告` 这类同主体显式依赖 / 前置条件矛盾，并用中英文防误报单测与支撑依赖直放行控制用例约束。
- [x] 已把 reviewer 从 lexical + structured-fact 检查继续扩展到确定性的 `claim_location_consistency`，覆盖 `位于主舱室` vs `位于辅助舱室` 这类同主体显式位置矛盾，并用中英文防误报单测与更宽位置直放行控制用例约束；`located in` / `位于` 这类位置谓词也已从 `claim_state_consistency` 抽取中排除，避免位置句误报成状态矛盾。
- [x] 已把 reviewer 从 lexical + structured-fact 检查继续扩展到确定性的 `claim_structured_comparison_consistency`，覆盖显式 `higher/lower`、`greater/less` 与 `高于/低于` 比较方向反转，并用中英文防误报单测与 mixed-property 控制用例约束。
...
- [x] 已新增确定性的 `claim_temporal_validity_consistency`：直接消费 `graphContext.temporalValidity`，对带时序警告却仍被当作“当前结论”发布的草稿执行改写，同时允许显式时间限定答案放行，并避免 `supersedes` 单独造成误报。
- [x] 已新增确定性的 `query_intent_alignment`：当 `what is` / `什么是` 类问题命中了定义证据，但草稿回答成了“本文档旨在……”这类文档自述时，reviewer 会在 release 前强制改写成直接定义句。
- [x] reviewer gate 发生变更时，运行时校验现在已显式要求先刷新编译产物；在执行 `scripts/verify-knowledge-workspace-runtime.js` 前先跑 `npm run build:mini`，避免陈旧 `dist` 掩盖新 gate。
- [ ] 在当前 lexical + query-intent + structured-fact + structured-comparison + attribute + containment + composition + purpose + dependency + location + subject + state + polarity + graph-causal + graph-order + graph-comparison + temporal-validity 门禁之上，继续扩展到更广的 claim-vs-citation / claim-vs-evidence 矛盾检测，并控制 false positive。
...
- [x] 已把命中的 evidence fragment 投影成 graph-focus 选中节点内的内联高亮，而不再只是整段/整块着色。
- [ ] 在当前 block-level markdown source metadata 与 snippet-projected 内联高亮基线之上，继续推进 source-authenticated 的字符级 provenance。
