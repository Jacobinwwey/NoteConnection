...
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_attribute_consistency` gate for same-subject explicit attribute contradictions such as `moderate thermal insulation` vs `high thermal insulation`, and cover it with English/Chinese false-positive-resistant unit cases plus a compatible-refinement control.
...
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_composition_consistency` gate for same-subject explicit composition contradictions such as `composed of water and a glass cup` vs `composed of oil and a plastic cup`, and cover it with English/Chinese false-positive-resistant unit cases plus a compatible-order control.
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_purpose_consistency` gate for same-subject explicit purpose contradictions such as `used for drinking water` vs `used for storing motor oil`, and cover it with English/Chinese false-positive-resistant unit cases plus a supported-purpose refinement control.
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_dependency_consistency` gate for same-subject explicit dependency/prerequisite contradictions such as `depends on Baseline Measurement and Sensor Calibration` vs `depends on Final Reporting`, and cover it with English/Chinese false-positive-resistant unit cases plus a supported-dependency release control.
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_location_consistency` gate for same-subject explicit location contradictions such as `located in the main chamber` vs `located in the auxiliary chamber`, cover it with English/Chinese false-positive-resistant unit cases, and keep locative predicates out of `claim_state_consistency` extraction so location claims do not misfire as state contradictions.
- [x] Extend the reviewer beyond lexical + structured-fact overlap with a deterministic `claim_structured_comparison_consistency` gate for explicit `higher/lower`, `greater/less`, and `高于/低于` inversions, and cover it with English/Chinese false-positive-resistant unit cases plus a mixed-property control.
...
- [x] Extend the reviewer beyond lexical + structured-fact + polarity + DAG-order checks with a deterministic `claim_graph_comparison_consistency` gate for `contrast` / `analogy` reversals against the assembled DAG, and cover it with false-positive-resistant English/Chinese unit cases.
- [x] Extend the reviewer with a deterministic `claim_temporal_validity_consistency` gate that consumes `graphContext.temporalValidity`, revises temporally flagged current-tense releases, allows explicit time-qualified answers, and avoids supersedes-only false positives.
- [x] Require fresh compiled runtime output before reviewer-gate runtime verification; `npm run build:mini` now precedes `scripts/verify-knowledge-workspace-runtime.js` when the live gate inventory changes, because stale `dist` can hide newly added gates.
- [ ] Extend contradiction coverage beyond the current lexical + query-intent + structured-fact + structured-comparison + attribute + containment + composition + purpose + dependency + location + subject + state + polarity + graph-causal + graph-order + graph-comparison + temporal-validity gates into broader claim-vs-citation / claim-vs-evidence conflicts without raising false positives.
