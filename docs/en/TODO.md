...
- [ ] Extend contradiction coverage beyond the current lexical + query-intent + structured-fact + attribute + containment + subject + state + polarity + graph-causal + graph-order + graph-comparison gates into broader claim-vs-citation / claim-vs-evidence conflicts without raising false positives.
