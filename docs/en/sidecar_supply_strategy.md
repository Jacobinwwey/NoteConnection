...
1. CI now seeds and prefers a project-controlled GitHub Releases mirror for Godot, but it still keeps upstream fallback and does not yet pin archive digests.
...
- The release workflow now also seeds standalone Godot mirror archives into a dedicated mirror tag before desktop bundle jobs run.
...
- remove upstream fallback and then remove Godot from legacy LFS once mirror/bootstrap parity is proven
