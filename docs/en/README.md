...
- The current gap is no longer “missing reviewer ownership,” and it is no longer “no provenance at all.” The remaining work is broader contradiction coverage beyond the current lexical + structured + state + polarity + graph-order stack, plus deeper exact-span / nested provenance beyond today's block-level markdown source mapping.
