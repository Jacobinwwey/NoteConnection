...
- The public-answer reviewer is now a real runtime owner, not a future idea: `src/learning/answerReleaseReview.ts` runs after scoped retrieval and graph-context assembly, strips internal diagnostics from the public surface, checks evidence sufficiency plus lexical/query-intent/structured/attribute/containment/subject/state/polarity/graph-causal/graph-order/graph-comparison consistency, and persists gate outcomes through conversation trace/export flows.
- The right-pane evidence preview now has a real source-to-render baseline rather than snippet-only guessing: `src/frontend/markdown_runtime.js` annotates rendered markdown blocks with source-line metadata, and `src/frontend/workspace_panes.js` prefers `source_line_provenance`, then trustworthy `line_window`, then `snippet_fallback`; once the correct node is selected, it now projects the matched evidence fragment into inline highlight markup instead of only tinting the whole paragraph.
- The screenshot-backed failure at `E:\微信传输\WeChat Files\wxid_zywfbxxiwwir22\FileStorage\Temp\1781782257390.jpg` is now a formal runtime acceptance case: `waterglass_explicit_scope_compact_zh` in `src/learning/KnowledgeWorkspaceConversationRegression.ts` requires at least one citation, forbids public leakage of `No scoped knowledge points matched` / `retrieval_candidates_below_threshold`, and now also forbids the meta-documentary answer fragment `本技术文档旨在`.
- The current gap is no longer “missing reviewer ownership,” and it is no longer “no provenance at all.” The remaining work is broader contradiction coverage beyond the current lexical + query-intent + structured + attribute + containment + subject + state + polarity + graph-causal + graph-order + graph-comparison stack, plus source-authenticated character-offset provenance beyond today's block-level markdown mapping and snippet-projected inline highlights.
- `ref/dspy`, `ref/guidance`, `ref/semantic-kernel`, `ref/langchain`, and `ref/litellm` remain design references, not runtime owners. The useful takeaways are typed LM subprograms, constrained output contracts, and evaluation loops. The rejected move is importing a Python-heavy or orchestration-heavy framework into a TypeScript DAG runtime that already owns retrieval, graph assembly, export, and UI compatibility; Semantic Kernel's current repo direction toward Microsoft Agent Framework and LiteLLM's provider-gateway role both reinforce that they are not substitutes for local release verification.
...
- Retrieval is now more graph-native than simple degree bonuses: `queryBackend.ts` uses bounded anchor-distance, directed-path-confidence, prerequisite-depth, temporal-invalidity, and relation-intent signals in both `local_hybrid` and `local_vector`.
...
- The next architecture move is graph-specific quality gates plus broader right-pane/operator diagnostics on top of the new assembler and graph-aware ranking boundaries.
