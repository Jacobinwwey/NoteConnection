...
The identity transition boundary now preflights the complete destination alias set before changing document, atom, or evidence records. Historical aliases remain compatibility lookups, so a URI, path, or basename already owned by another document fails closed. A valid move mirrors path/URI/revision/aliases into `ResourceRegistry`, the workspace binding, and `IndexLifecycle` in place; resource/projection/index IDs, legacy `documentId`, content hashes, and segments remain stable.

This closes the secondary-owner gap left by the earlier additive `sourceUri` work without changing snapshot/projection schemas, public IDs, or mobile runtime assets. It applies LearnGraph's typed-boundary discipline as an identity contract and keeps textbooks' package/compiler direction at the future ingestion boundary; neither reference warrants a database, Node sidecar, Godot runtime, or model in `mobile-slim`.

The new G4 fixtures prove rejected-collision state and successful four-owner convergence after persistence. Focused verification is 3 suites / 11 tests, TypeScript no-emit, and `git diff --check`; full regression is 148 Jest suites / 1,284 passed / 26 skipped, Rust 30 passed / 1 ignored, four-host projection replay passed, and fresh mobile-low budget passed. This is not full ingest transaction atomicity: a mixed request can still partially mutate before a later operation fails. Next is whole-request preflight or journaled rollback, then versioned G4 manifest archival; public-ID and SQLite/WASM promotion remain frozen until replay and native RSS evidence are reproducible.

## 2026-08-21 Phase 26 Request-Level Ingest Atomicity and Single-Writer Serialization

`ingestKnowledge` now uses a per-platform single-writer queue. Each request captures a deep pre-image of the existing versioned graph snapshot before mutation and restores document/atom/evidence maps, secondary registries, index state, identity journal, telemetry, and `idCounter` together after an operation, relation-recompute, owner-mirror, or atomic-persistence failure.

Identity ownership is checked at the boundary for both `upsert` and `move`. Path/URI/alias collisions, explicit move IDs whose `from*` aliases do not belong to the document, ambiguous source aliases, and missing owner mirrors fail closed. The mixed-batch G4 fixture proves that a successful first move is not visible after a later collision and that the original alias remains usable.
