...
- [ ] Next active task: extend contradiction coverage beyond the current lexical + query-intent + structured-fact + attribute + containment + composition + purpose + subject + state + polarity + graph-causal + graph-order + graph-comparison checks into broader claim/citation/evidence conflicts without letting false positives widen.
