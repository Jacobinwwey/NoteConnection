...
import { buildProviderProfileExport, parseProviderProfileImport, ProviderProfileExport, ProviderProfileImportSummary } from '../providerProfiles';
...
function resolveImportedActiveProvider(
    ...
    const providerNames = new Set(importedProviders.map(provider => provider.name as string));
    if (providerNames.has(activeProvider)) {
        return { activeProvider, activeProviderReset: false };
    }
    if (providerNames.has(defaultActiveProvider)) {
        return { activeProvider: defaultActiveProvider, activeProviderReset: true };
    }
    if (importedProviders.length > 0) {
        return { activeProvider: importedProviders[0].name, activeProviderReset: true };
    }
    return { activeProvider: defaultActiveProvider, activeProviderReset: true };
}
...
export async function executeExportProviderProfilesCommand(params: {
    ...
    const profile = buildProviderProfileExport(params.providers, params.now ?? new Date());
...
export async function executeImportProviderProfilesCommand(params: {
    ...
    const importSummary = parseProviderProfileImport(jsonData, params.existingProviders);
...
export async function executeExportCliCapabilityManifestCommand(params: {
    ...
    const manifest = buildCliCapabilityManifest(params.pluginId);
    const outputPath = await writePluginConfigJsonFile(
        params.host,
        params.pluginId,
