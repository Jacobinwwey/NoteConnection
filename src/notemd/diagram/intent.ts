...
export function inferDiagramIntent(markdown: string): DiagramIntentResult {
    ...
    if (hasMarkdownTable) chartReasons.push('markdown table detected');
    if (numericCells >= 2) chartReasons.push('numeric table values detected');
    if (chartKeywords > 0) chartReasons.push('metric-oriented keywords detected');
    if (shareKeywords > 0) chartReasons.push('part-to-whole keywords detected');
    if (percentValues >= 2) chartReasons.push('percentage values detected');
    if (comparisonKeywords > 0) chartReasons.push('comparison vocabulary detected');
    if (rankingKeywords > 0 && numericTokens >= 4) chartReasons.push('ranked numeric items detected');
    if (chartReasons.length >= 2) return scoreResult('dataChart', 0.88, chartReasons);
    ...
    if (sequenceKeywords >= 4) sequenceReasons.push('request-response vocabulary detected');
    if (/->|=>/.test(normalized)) sequenceReasons.push('directional interaction markers detected');
    if (sequenceReasons.length > 0) return scoreResult('sequence', 0.82, sequenceReasons);

    const erKeywords = countMatches(normalized, [
        /\bprimary key\b/g, /\bforeign key\b/g, /\bentity\b/g, /\btable\b/g,
        /\bcolumn\b/g, /\bschema\b/g
    ]);
    if (erKeywords >= 2) return scoreResult('erDiagram', 0.8, ['data-model vocabulary detected']);
    ...
    if (stateKeywords >= 3) return scoreResult('stateDiagram', 0.76, ['state-transition vocabulary detected']);
    ...
    if (canvasKeywords >= 1) return scoreResult('canvasMap', 0.72, ['spatial knowledge mapping vocabulary detected']);

    const flowchartReasons: string[] = [];
    const numberedSteps = (normalized.match(/^\s*\d+\./gm) ?? []).length;
    if (numberedSteps >= 2) flowchartReasons.push('numbered steps detected');
    ...
    if (flowKeywords >= 2) flowchartReasons.push('workflow/process vocabulary detected');
    if (flowchartReasons.length > 0) return scoreResult('flowchart', 0.74, flowchartReasons);
