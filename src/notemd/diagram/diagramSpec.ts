...
function collectNodeIds(nodes: DiagramNode[], ids: Set<string>, errors: string[]): void {
    nodes.forEach(node => {
        ...
        if (!id) { errors.push('Diagram node is missing an id.'); return; }
        if (ids.has(id)) { errors.push(`Diagram node id "${id}" is duplicated.`); }
        else { ids.add(id); }
        if (!node.label?.trim()) { errors.push(`Diagram node "${id}" is missing a label.`); }
        if (node.children?.length) { collectNodeIds(node.children, ids, errors); }
    });
}
...
function validateDataSeries(dataSeries: DiagramDataSeries[] | undefined, errors: string[]): void {
    ...
    dataSeries.forEach(series => {
        if (!series.id?.trim()) errors.push('Chart data series is missing an id.');
        if (!series.label?.trim()) errors.push(`Chart data series "${series.id || 'unknown'}" is missing a label.`);
        if (!series.points?.length) { errors.push(`Chart data series "${series.id || 'unknown'}" is missing points.`); return; }
...
export function validateDiagramSpec(spec: DiagramSpec): DiagramSpecValidationResult {
    ...
    if (!isSupportedDiagramIntent(spec.intent)) {
        errors.push(`Diagram spec uses unsupported intent "${String(spec.intent)}". Supported: ${SUPPORTED_DIAGRAM_INTENTS.join(', ')}.`);
    }
    if (!spec.title?.trim()) errors.push('Diagram spec title is required.');
    ...
    (spec.edges ?? []).forEach((edge, index) => {
        if (!nodeIds.has(edge.from)) errors.push(`Diagram edge ${index + 1} references missing source node "${edge.from}".`);
        if (!nodeIds.has(edge.to)) errors.push(`Diagram edge ${index + 1} references missing target node "${edge.to}".`);
    });
    if (spec.intent === 'dataChart') validateDataSeries(spec.dataSeries, errors);
    return { valid: errors.length === 0, errors };
}

export function assertValidDiagramSpec(spec: DiagramSpec): DiagramSpec {
    const result = validateDiagramSpec(spec);
    if (!result.valid) throw new ValidationError(result.errors.join(' '), 'INVALID_INPUT');
    return spec;
}
