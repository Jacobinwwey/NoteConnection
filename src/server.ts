    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'GET') {
            ...
            if (postPathname === '/api/knowledge/session/plan') {
                try {
                    const payload = await readJsonBody(req);
                    const requestPayload = payload as StudySessionRequest;
            ...
            if (postPathname === '/api/knowledge/session/action') {
                try {
                    const payload = await readJsonBody(req);
                    const requestPayload = payload as StudySessionActionExecutionRequest;
            ...
            if (postPathname === '/api/knowledge/session/history') {
                try {
                    const payload = await readJsonBody(req);
                    const requestPayload = payload as StudySessionHistoryRequest;
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:POST /api/knowledge/session/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
