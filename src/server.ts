...
export const startServer = async (options: { port?: number, targetPath?: string } = {}) => {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        if (req.method === 'GET') {
            ...
            if (getPathname === '/api/knowledge/tutor/catalog') {
                try {
                    const catalog = await knowledgeLearningPlatform.getTutorAdapterCatalog();
                    const settings = await loadNotemdSettings();
                    const providers = buildTutorProviderCatalog(settings);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/tutor/catalog');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/tutor/telemetry') {
                try {
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/tutor/telemetry');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/tutor/trace-diagnostics') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeTutorTraceDiagnosticsRequestFromQuery(urlObj.searchParams);
                    const result = await knowledgeLearningPlatform.queryTutorTraceDiagnostics(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/tutor/trace-diagnostics');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/tutor/trace-diagnostics/providers') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeTutorProviderTrendDiagnosticsRequestFromQuery(urlObj.searchParams);
                    const result = await knowledgeLearningPlatform.queryTutorProviderTrendDiagnostics(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/tutor/trace-diagnostics/providers');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/tutor/trace-diagnostics/providers/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeTutorProviderTrendHistoryRequestFromQuery(urlObj.searchParams);
                    const result = await knowledgeLearningPlatform.queryTutorProviderTrendHistory(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/tutor/trace-diagnostics/providers/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/query-backend-diagnostics') {
                try {
                    const queryBackendConfig = knowledgeLearningPlatform.getQueryBackendConfig();
                    ACTIVE_KNOWLEDGE_QUERY_BACKEND = queryBackendConfig.configuredBackend;
                    ...
                    res.end(
                        JSON.stringify({
                            success: true,
                            configuredBackend: ACTIVE_KNOWLEDGE_QUERY_BACKEND,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/query-backend-diagnostics');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/query-backend-config') {
                try {
                    const result = knowledgeLearningPlatform.getQueryBackendConfig();
                    const rolloutProfile = buildKnowledgeRuntimeRolloutProfile();
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/query-backend-config');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/query/compare-backends/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: KnowledgeQueryBackendComparisonHistoryRequest = {
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                    };
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/query/compare-backends/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/query/compare-backends/trend') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: KnowledgeQueryBackendComparisonTrendRequest = {
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                        windowSize: Number(urlObj.searchParams.get('windowSize') || 0) || undefined,
                        minSamples: Number(urlObj.searchParams.get('minSamples') || 0) || undefined,
                    };
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/query/compare-backends/trend');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/quality/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: LearningQualityHistoryRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                    };
                    const result = await knowledgeLearningPlatform.queryLearningQualityHistory(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/quality/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/session/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeStudySessionHistoryRequestFromQuery(urlObj.searchParams);
                    ...
                    writeApiErrorResponse(res, error, {
                        context: 'API:GET /api/knowledge/session/history',
                    });
                }
                return;
            }

            if (getPathname === '/api/knowledge/session/plan/quality/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: StudySessionPlanQualityHistoryRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                    };
                    const result = await knowledgeLearningPlatform.queryStudySessionPlanQualityHistory(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/session/plan/quality/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/session/plan/quality/trend') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: StudySessionPlanQualityTrendRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                        windowSize: Number(urlObj.searchParams.get('windowSize') || 0) || undefined,
                        minSamples: Number(urlObj.searchParams.get('minSamples') || 0) || undefined,
                    };
                    const result = await knowledgeLearningPlatform.queryStudySessionPlanQualityTrend(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/session/plan/quality/trend');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/session/plan/quality/thresholds/runtime') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const adaptiveEnabledRaw = String(urlObj.searchParams.get('adaptiveEnabled') || '').trim();
                    const adaptiveEnabled = adaptiveEnabledRaw
                        ? parseBooleanFlag(adaptiveEnabledRaw)
                        : STUDY_SESSION_PLAN_QUALITY_ADAPTIVE_THRESHOLDS_ENABLED;
                    const requestPayload: StudySessionPlanQualityRuntimeThresholdDiagnosticsRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        thresholds: {
                            ...STUDY_SESSION_PLAN_QUALITY_THRESHOLDS,
                        },
                        adaptiveThresholdsEnabled: adaptiveEnabled,
                        historyLimit: Number(urlObj.searchParams.get('historyLimit') || 0)
                            || RUNTIME_SESSION_PLAN_QUALITY_HISTORY_CONFIG.limit,
                        trendLimit: Number(urlObj.searchParams.get('trendLimit') || 0)
                            || RUNTIME_SESSION_PLAN_QUALITY_TREND_REQUEST.limit,
                        trendWindowSize: Number(urlObj.searchParams.get('trendWindowSize') || 0)
                            || RUNTIME_SESSION_PLAN_QUALITY_TREND_REQUEST.windowSize,
                        trendMinSamples: Number(urlObj.searchParams.get('trendMinSamples') || 0)
                            || RUNTIME_SESSION_PLAN_QUALITY_TREND_REQUEST.minSamples,
                    };
                    const result = await knowledgeLearningPlatform.queryStudySessionPlanQualityRuntimeThresholds(
                        requestPayload
                    );
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/session/plan/quality/thresholds/runtime');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/conversation-memory/list') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeConversationMemoryListRequestFromQuery(urlObj.searchParams);
                    const result = await knowledgeLearningPlatform.listConversationMemory(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    writeApiErrorResponse(res, error, {
                        context: 'API:GET /api/knowledge/conversation-memory/list',
                        requestId,
            ...
            if (getPathname === '/api/knowledge/conversation/turn-cache/diagnostics') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const pruneRequested = parseBooleanFlagOrUndefined(
                        urlObj.searchParams.get('prune')
                    ) === true;
                    const result = getAgentConversationTurnCacheDiagnostics({
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/conversation/turn-cache/diagnostics');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/conversation/turn-cache/diagnostics/trend') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeAgentConversationTurnCacheAlertTrendRequestFromQuery(
                        urlObj.searchParams
                    );
                    getAgentConversationTurnCacheDiagnostics({
                        prune: false,
                    });
                    const result = queryAgentConversationTurnCacheAlertTrend(requestPayload);
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/conversation/turn-cache/diagnostics/trend');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/conversation/turn-cache/diagnostics/trend/index') {
                try {
                    ...
                    const trendResult = queryAgentConversationTurnCacheAlertTrend(requestPayload);
                    appendRuntimeRunbookVerificationHistoryFromConversationTurnCacheAlertTrend(trendResult);
                    const result = {
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/conversation/turn-cache/diagnostics/trend/index');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/conversation/turn-cache/diagnostics/trend/export') {
                try {
                    ...
                    const trendResult = queryAgentConversationTurnCacheAlertTrend(requestPayload);
                    appendRuntimeRunbookVerificationHistoryFromConversationTurnCacheAlertTrend(trendResult);
                    const result = queryAgentConversationTurnCacheAlertTrendExport(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/conversation/turn-cache/diagnostics/trend/export');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/memory/policy/diagnostics') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: MemoryPolicyDiagnosticsRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        staleAfterHours: Number(urlObj.searchParams.get('staleAfterHours') || 0)
                            || RUNTIME_MEMORY_POLICY_DIAGNOSTICS_CONFIG.staleAfterHours,
                        nearExpiryHours: Number(urlObj.searchParams.get('nearExpiryHours') || 0)
                            || RUNTIME_MEMORY_POLICY_DIAGNOSTICS_CONFIG.nearExpiryHours,
                        lowConfidenceThreshold: Number(urlObj.searchParams.get('lowConfidenceThreshold') || NaN),
                        sampleLimit: Number(urlObj.searchParams.get('sampleLimit') || 0)
                            || RUNTIME_MEMORY_POLICY_DIAGNOSTICS_CONFIG.sampleLimit,
                    };
                    if (!Number.isFinite(Number(requestPayload.lowConfidenceThreshold))) {
                        requestPayload.lowConfidenceThreshold = RUNTIME_MEMORY_POLICY_DIAGNOSTICS_CONFIG.lowConfidenceThreshold;
                    }
                    const result = await knowledgeLearningPlatform.queryMemoryPolicyDiagnostics(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/memory/policy/diagnostics');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/staleness/diagnostics') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload = normalizeKnowledgeStalenessDiagnosticsRequestFromQuery(
                        urlObj.searchParams,
                        RUNTIME_KNOWLEDGE_STALENESS_DIAGNOSTICS_CONFIG.limit
                    );
                    const result = await knowledgeLearningPlatform.queryKnowledgeStalenessDiagnostics(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/staleness/diagnostics');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/memory/policy/history') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: MemoryPolicyDiagnosticsHistoryRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0)
                            || RUNTIME_MEMORY_POLICY_HISTORY_CONFIG.limit,
                    };
                    const result = await knowledgeLearningPlatform.queryMemoryPolicyDiagnosticsHistory(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/memory/policy/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/memory/policy/trend') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: MemoryPolicyDiagnosticsTrendRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0)
                            || RUNTIME_MEMORY_POLICY_TREND_REQUEST.limit,
                        windowSize: Number(urlObj.searchParams.get('windowSize') || 0)
                            || RUNTIME_MEMORY_POLICY_TREND_REQUEST.windowSize,
                        minSamples: Number(urlObj.searchParams.get('minSamples') || 0)
                            || RUNTIME_MEMORY_POLICY_TREND_REQUEST.minSamples,
                    };
                    const result = await knowledgeLearningPlatform.queryMemoryPolicyDiagnosticsTrend(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/memory/policy/trend');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/quality/trend') {
                try {
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const requestPayload: LearningQualityTrendRequest = {
                        userId: String(urlObj.searchParams.get('userId') || '').trim() || undefined,
                        limit: Number(urlObj.searchParams.get('limit') || 0) || undefined,
                        windowSize: Number(urlObj.searchParams.get('windowSize') || 0) || undefined,
                        minSamples: Number(urlObj.searchParams.get('minSamples') || 0) || undefined,
                    };
                    const result = await knowledgeLearningPlatform.queryLearningQualityTrend(requestPayload);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/quality/trend');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/quality/thresholds') {
                try {
                    const thresholds = knowledgeLearningPlatform.getLearningQualityThresholds();
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/quality/thresholds');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/session/orchestration/config') {
                try {
                    const trendRuntimeConfig = knowledgeLearningPlatform.getStudySessionOrchestrationTrendRuntimeConfig();
                    const memorySignalConfig = knowledgeLearningPlatform.getStudySessionOrchestrationMemorySignalConfig();
                    const tutorRoutingConfig = knowledgeLearningPlatform.getStudySessionOrchestrationTutorRoutingConfig();
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/session/orchestration/config');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-matrix') {
                try {
                    const runtimePayload = await buildKnowledgeRuntimePayload(new Date().toISOString());
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                            matrix: runtimePayload.runtimeCapabilityMatrix,
                            rolloutProfile: runtimePayload.rolloutProfile,
                        })
                    );
                } catch (error) {
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-matrix');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook') {
                try {
                    const runtimePayload = await buildKnowledgeRuntimePayload(new Date().toISOString());
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const checkId = String(urlObj.searchParams.get('checkId') || '');
                    const runbook = buildRuntimeCapabilityRunbook(
                        runtimePayload.runtimeCapabilityMatrix,
                        checkId
                    );
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-runbook');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/verify') {
                try {
                    const runtimePayload = await buildKnowledgeRuntimePayload(new Date().toISOString());
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const checkIdRawToken = String(urlObj.searchParams.get('checkId') || '').trim();
                    const checkIdRequestsAutoFocus = normalizeRuntimeRunbookVerificationFocusMode(checkIdRawToken)
                        === 'recommended';
                    const requestedCheckId = checkIdRequestsAutoFocus
                        ? ''
                        : normalizeRuntimeRunbookCheckIdToken(checkIdRawToken);
                    const focusModeFromParam = normalizeRuntimeRunbookVerificationFocusMode(
                        urlObj.searchParams.get('focus')
                    );
                    const focusMode: RuntimeRunbookVerificationFocusMode = requestedCheckId
                        ? 'none'
                        : (
                            focusModeFromParam === 'recommended' || checkIdRequestsAutoFocus
                                ? 'recommended'
                                : 'none'
                        );
                    const focusFilters = {
                        limit: parseRuntimeRunbookVerificationHistoryByCheckLimit(
                            urlObj.searchParams.get('focusLimit')
                        ),
                        sinceMinutes: parseRuntimeRunbookVerificationHistorySinceMinutes(
                            urlObj.searchParams.get('sinceMinutes')
                        ),
                        status: normalizeRuntimeRunbookVerificationStatusToken(
                            urlObj.searchParams.get('status')
                        ),
                        checkQuery: normalizeRuntimeRunbookVerificationHistoryCheckQueryToken(
                            urlObj.searchParams.get('checkQuery')
                        ),
                    };
                    let autoFocusApplied = false;
                    let autoFocusReason = 'not_requested';
                    let autoFocusRecommendedCheckId = '';
                    let autoFocusRecommendedFocusReason = 'none';
                    let resolvedCheckId = requestedCheckId;
                    if (focusMode === 'recommended') {
                        autoFocusReason = 'no_recommended_focus';
                        const checkSummary = queryRuntimeRunbookVerificationHistoryByCheck({
                            limit: focusFilters.limit,
                            sinceMinutes: focusFilters.sinceMinutes,
                            status: focusFilters.status,
                            checkQuery: focusFilters.checkQuery,
                            runtimeCapabilityMatrix: runtimePayload.runtimeCapabilityMatrix,
                        });
                        autoFocusRecommendedCheckId = normalizeRuntimeRunbookCheckIdToken(
                            checkSummary.summary.recommendedFocusCheckId
                        );
                        autoFocusRecommendedFocusReason = String(
                            checkSummary.summary.recommendedFocusReason || 'none'
                        );
                        if (autoFocusRecommendedCheckId) {
                            resolvedCheckId = autoFocusRecommendedCheckId;
                            autoFocusApplied = true;
                            autoFocusReason = autoFocusRecommendedFocusReason || 'recommended_focus';
                        }
                    }
                    const limit = parseRuntimeRequestTraceLimit(urlObj.searchParams.get('limit'));
                    const runbook = buildRuntimeCapabilityRunbook(
                        runtimePayload.runtimeCapabilityMatrix,
                        resolvedCheckId
                    );
                    ...
                    const traceSummary = queryRuntimeApiRequestTrace(traceQueryOptions).summary;
                    const selectedCheckStatusRaw = String(runbook.selectedCheck?.status || '').trim().toLowerCase();
                    ...
                    const verificationResultBase = {
                        ...
                        autoFocusApplied,
                        autoFocusReason,
                        autoFocusRecommendedCheckId,
                        autoFocusRecommendedFocusReason,
                        autoFocusFilters: {
                            ...focusFilters,
                        },
                        selectedCheckId: String(runbook.selectedCheck?.checkId || ''),
                        effectiveCheckId: String(runbook.selectedCheck?.checkId || ''),
                        selectedCheckStatus,
                        selectedCheckPriorityScore: Math.max(
                            0,
                            Math.floor(Number(runbook.selectedCheck?.priorityScore || 0))
                        ...
                        selectedCheckMessage: String(runbook.selectedCheck?.message || '').trim().slice(0, 280),
                        topRiskCheckId: String(runbook.topRiskCheck?.checkId || ''),
                        topRiskStatus,
                        traceSummary: {
                            ...traceSummary,
                        },
                        verificationTargets: runbook.verificationTargets || [],
                        runbook,
                    };
                    appendRuntimeRunbookVerificationHistoryRecord({
                        verifiedAt: String(verificationResultBase.generatedAt || new Date().toISOString()),
                        checkId: normalizeRuntimeRunbookCheckIdToken(verificationResultBase.selectedCheckId),
                        status: (
                            verificationResultBase.selectedCheckStatus === 'pass'
                            || verificationResultBase.selectedCheckStatus === 'warn'
                            || verificationResultBase.selectedCheckStatus === 'fail'
                        )
                            ? verificationResultBase.selectedCheckStatus
                            : 'unknown',
                        priorityScore: Math.max(0, Math.floor(Number(verificationResultBase.selectedCheckPriorityScore || 0))),
                        topRiskCheckId: normalizeRuntimeRunbookCheckIdToken(verificationResultBase.topRiskCheckId),
                        topRiskStatus: (
                            verificationResultBase.topRiskStatus === 'pass'
                            || verificationResultBase.topRiskStatus === 'warn'
                            || verificationResultBase.topRiskStatus === 'fail'
                        )
                            ? verificationResultBase.topRiskStatus
                            : 'none',
                        selectionSource: String(verificationResultBase.runbook?.selectionSource || 'none').trim().slice(0, 64),
                        traceSummary: {
                            returnedRecords: Math.max(0, Math.floor(Number(traceSummary.returnedRecords || 0))),
                            errorRequests: Math.max(0, Math.floor(Number(traceSummary.errorRequests || 0))),
                            errorRatioPct: Number(Number(traceSummary.errorRatioPct || 0).toFixed(4)),
                    ...
                    const selectedCheckIdToken = normalizeRuntimeRunbookCheckIdToken(
                        verificationResultBase.selectedCheckId
                    );
                    const selectedCheckHistorySinceMinutes = focusFilters.sinceMinutes > 0
                        ? focusFilters.sinceMinutes
                        : 1440;
                    const selectedCheckHistory = selectedCheckIdToken
                        ? queryRuntimeRunbookVerificationHistory({
                            limit: 200,
                            checkId: selectedCheckIdToken,
                            sinceMinutes: selectedCheckHistorySinceMinutes,
                            status: '',
                        }).summary
                        : {
                            returnedRecords: 0,
                    ...
                    const selectedCheckEscalation = resolveRuntimeRunbookVerificationEscalation(
                        ...
                        selectedCheckHistory.activeRiskStreak,
                        selectedCheckHistory.activeFailStreak,
                        selectedCheckIdToken
                    );
                    const dynamicModeSummary = queryRuntimeRunbookVerificationHistoryByCheck({
                        limit: 12,
                        sinceMinutes: selectedCheckHistorySinceMinutes,
                        status: '',
                        checkQuery: '',
                    }).summary;
                    const dynamicModeAlignmentLatestStatus = normalizeRuntimeRunbookVerificationStatusToken(
                        dynamicModeSummary.dynamicModeAlignmentLatestStatus
                    );
                    const dynamicModeAlignmentConflictStreak = Math.max(
                        0,
                        Math.floor(Number(dynamicModeSummary.dynamicModeAlignmentConflictStreak || 0))
                    );
                    const dynamicModeAlignmentFailStreak = Math.max(
                        0,
                        Math.floor(Number(dynamicModeSummary.dynamicModeAlignmentFailStreak || 0))
                    );
                    const dynamicModeAlignmentConflictPersistent = (
                        dynamicModeAlignmentConflictStreak >= 2
                        && isRuntimeRunbookVerificationRiskStatus(dynamicModeAlignmentLatestStatus || 'unknown')
                    );
                    const pathStrategyAlignmentLatestStatus = normalizeRuntimeRunbookVerificationStatusToken(
                        dynamicModeSummary.pathStrategyAlignmentLatestStatus
                    );
                    const pathStrategyAlignmentConflictStreak = Math.max(
                        0,
                        Math.floor(Number(dynamicModeSummary.pathStrategyAlignmentConflictStreak || 0))
                    );
                    const pathStrategyAlignmentFailStreak = Math.max(
                        0,
                        Math.floor(Number(dynamicModeSummary.pathStrategyAlignmentFailStreak || 0))
                    );
                    const pathStrategyAlignmentConflictPersistent = (
                        pathStrategyAlignmentConflictStreak >= 2
                        && isRuntimeRunbookVerificationRiskStatus(pathStrategyAlignmentLatestStatus || 'unknown')
                    );
                    const selectedCheckEscalationActionContext: Parameters<typeof resolveRuntimeRunbookVerificationEscalationActionItems>[0] = {
                        selectedCheckId: selectedCheckIdToken,
                        selectedCheckStatus: selectedCheckStatus,
                        selectedCheckEscalation,
                        selectedCheckHistory: {
                            returnedRecords: Math.max(
                                0,
                                Math.floor(Number(selectedCheckHistory.returnedRecords || 0))
                            ),
                            sinceMinutes: selectedCheckHistorySinceMinutes,
                            activeRiskStreak: Math.max(
                                0,
                                Math.floor(Number(selectedCheckHistory.activeRiskStreak || 0))
                            ),
                            activeFailStreak: Math.max(
                                0,
                                Math.floor(Number(selectedCheckHistory.activeFailStreak || 0))
                            ),
                            trendStatus: (
                                selectedCheckHistory.trendStatus === 'improving'
                                || selectedCheckHistory.trendStatus === 'stable'
                        ...
                        dynamicModeAlignment: {
                            conflictPersistent: dynamicModeAlignmentConflictPersistent,
                            conflictStreak: dynamicModeAlignmentConflictStreak,
                            failStreak: dynamicModeAlignmentFailStreak,
                            recommendedFocusCheckId: String(
                                dynamicModeSummary.recommendedFocusCheckId || ''
                            ),
                            recommendedFocusReason: String(
                                dynamicModeSummary.recommendedFocusReason || 'none'
                            ),
                        },
                        pathStrategyAlignment: {
                            conflictPersistent: pathStrategyAlignmentConflictPersistent,
                            conflictStreak: pathStrategyAlignmentConflictStreak,
                            failStreak: pathStrategyAlignmentFailStreak,
                            recommendedFocusCheckId: String(
                                dynamicModeSummary.recommendedFocusCheckId || ''
                            ),
                            recommendedFocusReason: String(
                        ...
                        verificationTargets: Array.isArray(verificationResultBase.verificationTargets)
                            ? verificationResultBase.verificationTargets
                            : [],
                    };
                    const selectedCheckEscalationActionItems = resolveRuntimeRunbookVerificationEscalationActionItems(
                        selectedCheckEscalationActionContext
                    );
                    const selectedCheckEscalationActions = selectedCheckEscalationActionItems
                        .map((item) => String(item?.instruction || '').replace(/\s+/g, ' ').trim())
                        .filter(Boolean)
                        .slice(0, 8);
                    const verificationResult = {
                        ...
                        selectedCheckHistory: {
                            ...
                            trendStatus: selectedCheckHistory.trendStatus || 'insufficient_data',
                            trendWindowSize: Math.max(
                                0,
                                Math.floor(Number(selectedCheckHistory.trendWindowSize || 0))
                            ),
                            severityDelta: Number(Number(selectedCheckHistory.severityDelta || 0).toFixed(4)),
                            latestVerifiedAt: String(selectedCheckHistory.latestVerifiedAt || ''),
                        },
                        selectedCheckRemediation,
                        selectedCheckEscalation,
                        ...
                        dynamicModeAlignment: {
                            records: Math.max(
                                0,
                                Math.floor(Number(dynamicModeSummary.dynamicModeAlignmentRecords || 0))
                            ),
                            latestStatus: dynamicModeAlignmentLatestStatus || '',
                            conflictStreak: dynamicModeAlignmentConflictStreak,
                            failStreak: dynamicModeAlignmentFailStreak,
                            conflictPersistent: dynamicModeAlignmentConflictPersistent,
                            recommendedFocusCheckId: String(
                        ...
                        pathStrategyAlignment: {
                            records: Math.max(
                                0,
                                Math.floor(Number(dynamicModeSummary.pathStrategyAlignmentRecords || 0))
                            ),
                            latestStatus: pathStrategyAlignmentLatestStatus || '',
                            conflictStreak: pathStrategyAlignmentConflictStreak,
                            failStreak: pathStrategyAlignmentFailStreak,
                            conflictPersistent: pathStrategyAlignmentConflictPersistent,
                            recommendedFocusCheckId: String(
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-runbook/verify');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/history') {
                try {
                    const rolloutProfile = buildKnowledgeRuntimeRolloutProfile();
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const queryOptions = normalizeRuntimeRunbookVerificationHistoryQueryOptions(
                        urlObj.searchParams
                    );
                    const result = queryRuntimeRunbookVerificationHistory(queryOptions);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-runbook/history');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/history/checks') {
                try {
                    const runtimePayload = await buildKnowledgeRuntimePayload(new Date().toISOString());
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const queryOptions = normalizeRuntimeRunbookVerificationHistoryByCheckQueryOptions(
                        urlObj.searchParams
                    );
                    const result = queryRuntimeRunbookVerificationHistoryByCheck({
                        ...queryOptions,
                        runtimeCapabilityMatrix: runtimePayload.runtimeCapabilityMatrix,
                    });
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-runbook/history/checks');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/history/action-queue') {
                try {
                    const rolloutProfile = buildKnowledgeRuntimeRolloutProfile();
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const queueQuery = normalizeRuntimeRunbookVerificationActionQueueQueryOptions(
                        urlObj.searchParams
                    );
                    const result = queryRuntimeRunbookVerificationActionQueue({
                        checksQuery: queueQuery.checksQuery,
                        queueLimit: queueQuery.queueLimit,
                        priorityFilter: queueQuery.priorityFilter,
                        categoryFilter: queueQuery.categoryFilter,
                        checkIdFilter: queueQuery.checkIdFilter,
                        remediationStatusFilter: queueQuery.remediationStatusFilter,
                        remediationTrendFilter: queueQuery.remediationTrendFilter,
                    });
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                    ...
                    console.error(error);
                    CrashLogger.log(error, 'API:GET /api/knowledge/runtime-capability-runbook/history/action-queue');
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
                return;
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/history/remediation-events') {
                try {
                    const rolloutProfile = buildKnowledgeRuntimeRolloutProfile();
                    const urlObj = new URL(req.url || '/', `http://${LOOPBACK_HOST}:${finalPort}`);
                    const queryOptions = normalizeRuntimeRunbookRemediationEventQueryOptions(
                        urlObj.searchParams
                    );
                    const result = queryRuntimeRunbookRemediationEventHistory(queryOptions);
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    CrashLogger.log(
                        error,
                        'API:GET /api/knowledge/runtime-capability-runbook/history/remediation-events'
                    );
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
            ...
            if (getPathname === '/api/knowledge/runtime-capability-runbook/remediation-event/replay-schedule') {
                try {
                    const rolloutProfile = buildKnowledgeRuntimeRolloutProfile();
                    const result = getRuntimeRunbookRemediationReplayScheduleSnapshot();
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                    res.end(
                        JSON.stringify({
                            success: true,
                    ...
                    CrashLogger.log(
                        error,
                        'API:GET /api/knowledge/runtime-capability-runbook/remediation-event/replay-schedule'
                    );
                    res.writeHead(500, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: String(error) }));
                }
