...
- `关联聚焦` 已实现为右侧 pane 内托管的 Focus-mode：复用主图投影契约与共享双击决策契约，在 pane 内切换 anchor 或打开 Markdown，不移动主图 DOM，也不调用全局 reader；托管视图刻意不显示主界面的工具框和可见边线。`学习路径` 托管 Godot Future Path 的 `diffusion/core/treeLayout` 契约，并将 DOM 输入路由为 TreeRenderer 风格信号，包括对选中节点的右键 prerequisite 收起。
